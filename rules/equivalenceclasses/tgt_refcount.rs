...
// See src.cpp and tgt_unsafe.rs.  EquivalenceClasses<T1> is a FLAT LEADER MAP, BTreeMap<T1, T1>
...
// Nothing in this module stores or hands out a pointer or a reference into
// container-owned storage -- every mapped body returns a bool or a cloned
// value -- so the refcount and unsafe targets are identical.  The surface that
// WOULD differ between them (getLeaderValue, *MI, ECValue) is exactly the
// surface src.cpp leaves unmapped.
