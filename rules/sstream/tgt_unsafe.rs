...
unsafe fn f30<'a>(a0: &'a mut Box<libcc2rs::StringStream>, a1: &mut Vec<libc::c_char>, a2: libc::c_char) -> &'a mut Box<libcc2rs::StringStream> {
    ({
        let __line: Vec<u8> = libcc2rs::Cc2Extract::cc2_getline(&mut *a0, a2 as u8);
...
unsafe fn f31<'a>(a0: &'a mut Box<libcc2rs::StringStream>, a1: &mut Vec<libc::c_char>) -> &'a mut Box<libcc2rs::StringStream> {
    ({
        let __line: Vec<u8> = libcc2rs::Cc2Extract::cc2_getline(&mut *a0, b'\n');
        let mut __out: Vec<libc::c_char> = __line.iter().map(|&b| b as libc::c_char).collect();
        __out.push(0);
        *a1 = __out;
        libcc2rs::stream_mut(&mut *a0)
