...
pub unsafe fn copy_or_move_into_param_4(mut t: Tracked) -> Tracked {
    return Tracked::move_from({ &mut t });
}
pub fn main() {
    unsafe {
...
unsafe fn main_0() -> i32 {
    ...
    assert!(
        (((unsafe { chosen_overload_0(&(*&mut a),) }) as i32) == (Overload_kLvalueOverload as i32))
    );
    assert!(((a.v) == (5)));
    assert!(
        (((unsafe { chosen_overload_1(&mut (*&mut a),) }) as i32)
            == (Overload_kRvalueOverload as i32))
    );
    assert!(((a.v) == (5)));
    let mut b: Tracked = Tracked::new({ 6 });
    let mut moved: Tracked =
        (unsafe { copy_or_move_into_param_4(Tracked::move_from({ &mut (*&mut b) })) });
    assert!(((moved.v) == (6)));
    ...
    let mut copied: Tracked =
        (unsafe { copy_or_move_into_param_4(Tracked::copy_from({ &(*&mut c) })) });
    assert!(((copied.v) == (7)));
    ...
    let mut i: i32 = 8;
    assert!(
        (((unsafe { chosen_overload_2(&(*&mut i),) }) as i32)
            == (Overload_kIntLvalueOverload as i32))
    );
    assert!(
        (((unsafe { chosen_overload_3(&mut (*&mut i),) }) as i32)
