...
pub unsafe fn DoStuffWithSafePointer_0(safe_ptr: *mut Option<Box<SafePointer>>) {
    let mut x1: Option<Box<i32>> = (unsafe {
        let mut ___args: i32 = 0;
        libcc2rs::make_unique_unsafe(&mut ___args)
    });
    let mut x2: Option<Box<i32>> = (unsafe {
        let mut ___args: i32 = 0;
        libcc2rs::make_unique_unsafe(&mut ___args)
    });
    ...
    (unsafe { SafePointer::inc(&mut (*(*safe_ptr).as_deref_mut().unwrap())) });
    let mut x3: Option<Box<i32>> = (unsafe {
        let mut ___args: i32 = 10;
        libcc2rs::make_unique_unsafe(&mut ___args)
    });
    let mut x4: Option<Box<i32>> = (unsafe {
        let mut ___args: i32 = 20;
        libcc2rs::make_unique_unsafe(&mut ___args)
    });
    ...
    (*raw_ptr2) += 1;
    let mut pair: Option<Box<Pair>> = (unsafe {
        let mut ___args: Pair = Pair {
            x: (*raw_ptr2),
            y: 5,
        };
        libcc2rs::make_unique_unsafe(&mut ___args)
...
unsafe fn main_0() -> i32 {
    let mut x: Option<Box<i32>> = (unsafe {
        let mut ___args: i32 = 0;
        libcc2rs::make_unique_unsafe(&mut ___args)
    });
    let mut safe_ptr: Option<Box<SafePointer>> = (unsafe {
        let mut ___args: SafePointer = SafePointer { ptr: x.take() };
        libcc2rs::make_unique_unsafe(&mut ___args)
    });
